...
solarIrr = sol(wl);
p = Legendre([2.0,0.0001,0.000001,0.0001,0.000001,0.0001,0.000001,0.0001,0.000001,0.0001]);
x2 = [vmr_co2; vmr_h2o; vmr_ch4; vmr_co; vmr_n2o; vmr_hdo;vmr_co2; 1e-9*ones(10); p[:] ];
...
lociBoxA = CarbonI.KernelInstrument(kernf, collect(2040:SSI:2100));
lociBoxB = CarbonI.KernelInstrument(kernf, collect(2100:SSI:2380));
lociBox = lociBoxB
...
# Handmade to do ABSCO table in SCO2
y1 = y
F1 = F_all[:,end]
y2 = y
F2 = F_all[:,end]
y = [y1; y2]
F = [F1; F2]
ν = [lociBoxA.ν_out; lociBoxB.ν_out]

f = Figure(resolution=(500,400))
ax = Axis(f[1,1], title="Carbon-I Fit to TCCON Spectrum", ylabel="Radiance [AU]")
ax_ = Axis(f[2,1],xlabel="Wavenlength [nm]",ylabel="Residual (%)" )
ax2 = lines!(ax,ν, F,label="Carbon-I fit")
lines!(ax,ν, y, label="Carbon-I spectrum (convolved TCCON data)")
lines!(ax_,ν, (F-y)/0.25*100, label="Residual (%)")
CairoMakie.ylims!(ax,0.08,0.28)
CairoMakie.ylims!(ax_,-2,2)
CairoMakie.xlims!(ax,2040,2380)
CairoMakie.xlims!(ax_,2040,2380)
axislegend(ax, position=:cb, framevisible=false)
#axislegend(ax_,position=:rt)
hidexdecorations!(ax, grid=false)
rowsize!(f.layout,1,Relative(0.7))
rowgap!(f.layout,0)
f
CairoMakie.save("CarbonI_fit.pdf", f)
