...
function plotCloudFractional()
    ...
    ax1 = Axis(f[1, 1];
        ...
        ylabel="Retrieved XCH₄ using N₂O Proxy method (ppb)",title="Impact of thin Cirrus Cloud at 200hPa",
...
save("plots/ProxyPaper/FractionalCloudsHighCirrus.pdf", f)

function plotCloudFractionalH()
    f = Figure(resolution=(500,400), title="Impact of fractional cloud cover on boundary layer enhancements", fontsize=16)

    # Primary axis (left Y)
    ax1 = Axis(f[1, 1];
        xlabel="Cloud Fraction (%)",
        ylabel="Retrieved normalized H₂O column",title="Thin Cirrus Cloud at 200hPa",
        yminorgridvisible=true,xticks=0:10:100,
        #backgroundcolor=:transparent
    )
...
f = plotCloudFractionalH()
save("plots/ProxyPaper/FractionalCloudsHighCirrusWater.pdf", f)