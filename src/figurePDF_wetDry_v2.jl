cmap = cgrad(:PuBuGn_9,  15, categorical = true)
...
data_ = data
data_dry = data_[:,1]
hm = CairoMakie.heatmap!(ax, x2, [0.8e-3; bins[2:end]], data_,colormap=cmap,ylims=(1e-3,1),xlims = (0, 2000), colorrange=(0,0.06), alpha=0.7)# Add a colorbar
...
axislegend(ax2, [mean_line,med_line], ["Mean", "Median"],  position = :rt,
...
ax2.yticks = ([0.001,0.002, 0.005, 0.01,0.02, 0.05, 0.1, 0.2, 0.5, 1], ["0.1","0.2", "0.5", "1","2", "5", "10","20", "50","100"])
...
#colsize!(fig.layout, 2, Relative(0.47))
fig
CairoMakie.save("Cloud_WetDrySeason_S2_v2.pdf", fig)
