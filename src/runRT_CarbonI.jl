...
parameters = parameters_from_yaml("src/carbon-i.yaml]")
...
R_conv = CarbonI.conv_spectra(lociBox, wl, rad_inter(1e7./wl))


# Run medium AOD case:
parameters = parameters_from_yaml("src/carbon-i-mediumAOD.yaml")
model = model_from_parameters(parameters);
a2 = rt_run(model)
...
# Run N2O case:
parameters = parameters_from_yaml("src/carbon-i-dN2O.yaml")
model = model_from_parameters(parameters);
a3 = rt_run(model)
