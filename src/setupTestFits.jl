...
x = [vmr_co2; vmr_h2o; vmr_ch4;vmr_co; vmr_n2o; 0.1e-9*vmr_hdo; vmr_co2; vmr_c2h6;   p[:] ];
...
rel_error = 0.1;
correlationMatrix = CarbonI.createCorrelationMatrix(profile.p/1e2,250.0,650)
correlationMatrix_h2o = CarbonI.createCorrelationMatrix(profile.p/1e2,250.0,100)
...
e_h2o  = CarbonI.createErrorVector(profile.p/1e2,250.0,850.0, 10*rel_error, vmr_h2o)
e_n2o  = CarbonI.createErrorVector(profile.p/1e2,250.0,850.0, rel_error, vmr_n2o)
e_co   = CarbonI.createErrorVector(profile.p/1e2,250.0,850.0, rel_error*5, vmr_co)
e_hdo  = CarbonI.createErrorVector(profile.p/1e2,250.0,850.0, rel_error, vmr_hdo)
e_c2h6 = CarbonI.createErrorVector(profile.p/1e2,250.0,850.0, rel_error*5, vmr_c2h6)
...
x = [vmr_co2; vmr_h2o; vmr_ch4; vmr_co; vmr_n2o; 0.1e-10*vmr_hdo;vmr_co2;vmr_c2h6;   p[:] ];
...
#for i=1:200
for albedo = 0.02:0.02:0.45
 for aodd = 0.0:0.005:0.1
...
for i=1:max_no_of_iter
    ...
    println("Column averaged CO₂: ", (h_co2' * x_all[:,i+1] * 1e6)/400)
    println("Column averaged 13CO₂: ", (h_co2_' * x_all[:,i+1] * 1e6)/400)
    println("Column averaged N₂O: ", (h_n2o' * x_all[:,i+1] * 1e6)/0.32)
    println("Column averaged CH₄: ", (h_ch4' * x_all[:,i+1] * 1e9)/2000)
    println("Column averaged C2H6: ", (h_c2h6' * x_all[:,i+1] * 1e9)/1.0)
    println("Column averaged H2O: ", (h_h2o' * x_all[:,i+1] * 1e6)/5313.10461)
